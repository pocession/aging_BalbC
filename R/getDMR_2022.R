...
# 5hmc + 5mc ====
sampleSheetDf <- sampleSheetTotal
...
for (i in 1:length(group_list)) {
  index <- as.character(group_list[[i]])
  write.csv(dmrObjList[[index]], file.path(outputTmpDir, paste0("DMR_total_", YEAR, "_", TISSUE, "_", index, ".csv")), row.names = FALSE, quote = FALSE)
}
remove(i, index)
